...
update_history_values = function()
{
	...
	gyro_x_history.unshift(ecg_data.gyro_x);
	gyro_y_history.unshift(ecg_data.gyro_y);
	gyro_z_history.unshift(ecg_data.gyro_z);
...
decode_8_16 = function(val) //decodes 8-bit compressed value into signed 16 bit
{
	var pow4 = (val>>4)&0b111;
	var is_negative = val>>7;
	var rr = val&0xF;
	var r1 = 1<<(pow4*2);
	var r2 = rr;
	var res = 2*r1 * (0.25 + r2/20.0);
	if(is_negative > 0) return -res;
	return res;
}
...
const param2_batt_bpm = 0;
const param2_sdnn = 1;
const param2_skin_res = 2;
const param2_lastRR = 3;
const param2_imu_acc = 4;
const param2_imu_gyro = 5;
const param2_imu_steps = 6;
const param2_pnn_bins = 7;
const param2_end = 8;
const param2_emg_spectrum = 9;
...
parse_uecg_packet_v2 = function(pack)
{
	...
	var message_length = pack[1];
//	if(message_length > maxlen) return -1; //not enough received data
//	console.log("len " + message_length + " pack id " + packet_id);
	var ppos = 2;
	...
	var data_points = pack[ppos++]-50;
	if(data_points < 6) return ; //at least 6 data points per valid uECG packet
	if(data_points > 31) return ; //too many for v2 version

	protocol_v6p = 1;

	var data_id = pack[ppos++];

	var uecg_res = [];
	uecg_res.ID = unit_id;

	var param_id = pack[ppos++]; //parameter ID

	if(param_id > param_emg_spectrum) return; //unknown parameter
...
//check for data integrity
	...
	{
		var val = pack[ppos+x*2]*256 + pack[ppos+x*2+1];
		if(isNaN(val)) return;
	}
	...
	if(unit_index != 0) return; //for now
...
//		printf("duplicate %d\n", packet_id);
	...
	var prev_data_id = unit_list[unit_index].data_id;
	var data_dist = data_id - prev_data_id;
	if(prev_data_id > 150 && data_id < 120)
		data_dist = data_id + (256-prev_data_id);
...
//	console.log("prev_data_id " + prev_data_id + " data_id " + data_id + " data_dist " + data_dist);
	...
	if(param_id == param2_batt_bpm) //battery level and BPM data
	{
	...
	if(param_id == param2_lastRR) //2 RR intervals: current and previous
	...
	if(param_id == param2_skin_res) //skin resistance, translation into kOhms will be provided later - not obvious even for us :)
	{
	...
	if(param_id == param2_imu_steps) //step counter, overflows at 65536
	{
		uecg_res.steps = (param_hb<<8) | param_lb;
	}
	if(param_id == param2_imu_acc) //current acceleration by x,y,z packed in a way to increase precision around 0, so angle can be extracted more or less reliably
	{
		uecg_res.acc_x = decode_acc(param_hb);
		uecg_res.acc_y = decode_acc(param_lb);
		uecg_res.acc_z = decode_acc(param_tb);
	}
	if(param_id == param2_imu_gyro) //current acceleration by x,y,z packed in a way to increase precision around 0, so angle can be extracted more or less reliably
	{
		uecg_res.gyro_x = decode_8_16(param_hb);
		uecg_res.gyro_y = decode_8_16(param_lb);
		uecg_res.gyro_z = decode_8_16(param_tb);
	}

	uecg_res.RR_data = Array();

	if(data_dist > data_points)
	{
		var val = pack[ppos]*256 + pack[ppos+1];
		if(val > 32767) val = -65535 + val;
		for(var x = 0; x < data_dist - data_points; x++)
...
//	if(message_length > maxlen) return -1; //not enough received data
	...
	for(var x = 0; x < unit_list.length; x++) //check if ID is in the list
		if(unit_list[x].ID == unit_id)
		{
			if(unit_list[x].protocol_version == 6) return parse_uecg_packet_v2(pack);
		}
	...
	if(data_points > 31)
	{
		if(data_points > 56 && data_points < 82) return parse_uecg_packet_v2(pack);
		return ;
	}
		...
		{//we detected possible start of the packet, trying to make sense of it
			...
			else
			{
				...
				for(var x = 0; x < res.RR_data.length; x++)
				{
					...
					ecg_data_queue[0] = global_data_id;
					global_data_id++;
				}
...
app.get('/get-params-history',
  ...
  uecg_hist.h_gyro_x = gyro_x_history.slice(0, history_pos - last_hist);
  uecg_hist.h_gyro_y = gyro_y_history.slice(0, history_pos - last_hist);
  uecg_hist.h_gyro_z = gyro_z_history.slice(0, history_pos - last_hist);
...
//	console.log("RP: " + response_pos);
	for(var x = 0; x < response_pos-3-fw_upload_RF*5; x++) 
	{
			...
			if(response_buf[x] == uart_prefix_serial[0] && response_buf[x+1] == uart_prefix_serial[1])
		{
			...
			console.log("pack id rcvd " + pack_id + " expected " + upload_pack_id);
//			console.log("last_err_code " + last_err_code + " response_pos " + response_pos + " x " + x);
			...
			else
			{
				...
				{
					console.log("chk err: odd " +  response_buf[x+4] + " tri " + response_buf[x+5] + " all " + response_buf[x+6]);
				}
			}
			processed_pos = x + 3;
		}
...
//			printf("radio pack: %d %d\n", pack_id, ret_code);
			...
			else if(pack_id == upload_pack_id && ret_code == err_code_packmiss || pack_id != upload_pack_id)
			{
				response_pending = 2;
				need_resend = 0;
			}
...
send_data_radio = function(data, len)
{
	...
	console.log("sending data, checks: odd " + check_odd + " tri " + check_tri + " all " + check_all);
